  ...
  useEffect(() => {
    const performSearch = async () => {
      ...
      try {
        ...
        const ingredients: SearchResult[] = ingredientsResponse.ingredients.map(
          (ingredient: Ingredient) => ({
            ...
            carbs_per_100g:
              ingredient.macros_per_100g_or_ml?.carbohydrates || 0,
            ...
            saturated_fat_per_100g:
              ingredient.macros_per_100g_or_ml?.saturated_fat || 0,
            tags: ingredient.tags || [],
          })
        );

        const products: SearchResult[] = productsResponse.products.map(
          (product: Product) => ({
            id: product.id,
            name: product.name,
            ...
            saturated_fat_per_100g:
              product.macros_per_100g_or_ml?.saturated_fat || 0,
            tags: product.tags || [],
            brand: product.brand,
          })
        );
  ...
  const handleAddIngredient = () => {
    ...
    const isAlreadyAdded = selectedIngredients.some(
      (ingredient) =>
        ...
        ingredient.type === selectedItem.type
    );
            ...
            <Table>
              ...
              <Table.Tbody>
                {selectedIngredients.map((ingredient, index) => (
                  <Table.Tr
                    key={`${ingredient.type}-${ingredient.id}-${index}`}
                  >
                    <Table.Td>
                      <Text size="sm" fw={500}>
                        {ingredient.name}
                    ...
                    <Table.Td>
                      <Badge
                        ...
                        variant="light"
                        color={
                          ingredient.type === "ingredient" ? "green" : "blue"
                        }
                      >
                        {ingredient.type}
                      </Badge>
                    ...
                    <Table.Td>
                      <NumberInput
                        ...
                        onChange={(value: any) =>
                          handleUpdateQuantity(index, Number(value) || 0)
                        }
                        min={0.1}
                        step={0.1}
                        decimalScale={1}
                    ...
                    <Table.Td>
                      <Select
                        ...
                        onChange={(value: string | null) =>
                          handleUpdateUnit(index, value || "g")
                        }
                        data={UNIT_OPTIONS}
                        size="xs"
                        w={100}
                    ...
                    <Table.Td>
                      <Text size="sm">
                        {Math.round(
                          (ingredient.calories_per_100g * ingredient.quantity) /
                            100
                        )}{" "}
                        cal
                      </Text>
