...
export const DateRangeSelectorForShopping: React.FC<DateRangeSelectorForShoppingProps> = ({
    startDate,
    endDate,
    onDateRangeChange,
    onFiltersChange,
        ...
        <Stack gap="md">
            ...
            {isDateRangeValid && (
                <Alert icon={<IconInfoCircle size={16} />} color="blue" variant="light">
                    <Text size="sm">
                        Selected period: <strong>{daysDifference} day{daysDifference !== 1 ? 's' : ''}</strong>
            ...
            {isDateRangeValid && (
                <Stack gap="xs">
                    {isLoadingPreview && (
                        <Group>
                            <Loader size="sm" />
                            <Text size="sm" color="dimmed">Loading meal plan preview...</Text>
                        </Group>
                    )}
                    ...
                    {preview && !isLoadingPreview && (
                        <Alert icon={<IconInfoCircle size={16} />} color="green" variant="light">
                            <Text size="sm">
                                Found meals planned for <strong>{Object.keys(preview.meal_assignments).length} days</strong> in this period.
                                {Object.keys(preview.meal_assignments).length === 0 && (
                                    <Text color="orange" size="sm">
                                        No meal plans found for this date range. The shopping list will be empty.
                                    </Text>
                                )}
                            </Text>
                        </Alert>
            ...
            <Collapse in={showAdvanced}>
                <Stack gap="md" p="md" style={{ border: '1px solid #e9ecef', borderRadius: 4 }}>
                    <Text size="sm" fw={600}>Filter Options</Text>
                    ...
                    {filters.excludeMealTypes.length > 0 && (
                        <Group gap="xs">
                            <Text size="sm" color="dimmed">Excluding:</Text>
                            {filters.excludeMealTypes.map((mealType) => (
                                <Badge key={mealType} size="sm" variant="light" color="red">
                                    {MEAL_TYPE_OPTIONS.find(opt => opt.value === mealType)?.label || mealType}
                                </Badge>
                            ))}
                        </Group>
                    )}
