...
class ShoppingListGenerateRequest(BaseModel):
    ...
    exclude_meal_types: Optional[list[str]] = Field(
        default=None, description="Meal types to exclude from the shopping list"
    )
...
class ExportShoppingListRequest(BaseModel):
    ...
    format: str = Field(
        ..., description="Export format (csv, pdf)", pattern="^(csv|pdf)$"
    )
    shopping_list: ShoppingListResponse = Field(
        ..., description="The shopping list to export"
    )
